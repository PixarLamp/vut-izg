...
class GPU{
  ...
  private:
    ...
    BufferID buffer;
    uint8_t* address;
    std::map<BufferID, uint8_t*> buffers;
    ...
    struct verpul {
        ...
        struct indexing {
            BufferID buffer = emptyID;
            IndexType type;
        }index;

    };
    std::map<VertexPullerID, verpul> vertextable;
    verpul tab;
    ...
    std::map<ProgramID, programset> programs;
    programset neededprog;
